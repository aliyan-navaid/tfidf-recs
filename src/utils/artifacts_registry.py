...
class ArtifactsRegistry:
    ...
    def _find_version_candidates(self) -> list:
        pattern = re.compile(
            rf"^{self.base_version}\.(\d{{8}}_\d{{6}})$"
        )
        return [
            entry.name
    ...
    def register_artifact(
        ...
        artifact_path = (
            self.base_path / self.active_version / artifact_type
        )
    ...
    def get_artifact(
        ...
        return self._load_artifact_data(
            version.artifacts[artifact_type][artifact_name],
            artifact_type
        )
    ...
    def _load_artifact_data(
        ...
        if not artifact_meta.path.exists():
            raise FileNotFoundError(
                f"Artifact file not found: {artifact_meta.path}"
            )
    ...
    def commit(self) -> None:
        ...
        self.pending_artifacts = {
            k: v for k, v in self.pending_artifacts.items()
    ...
    def _validate_dependencies(
        ...
        for dep_type in dependencies:
            if dep_type not in version.artifacts or \
               not version.artifacts[dep_type]:
    ...
    def _validate_required_artifacts(
        ...
        for artifact_type, config in self.artifacts_config.items():
            if config.required:
                if artifact_type not in version.artifacts or \
                   not version.artifacts[artifact_type]:
